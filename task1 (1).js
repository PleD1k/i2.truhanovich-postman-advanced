...
function updateText(textToSplit, separator) {
  const backslashes = textToSplit.match(/\\/g);
  console.log(`Number of backslashes: ${backslashes ? backslashes.length : 0}`);
  const updatedText = textToSplit.replace(/\\/g, '');
  return updatedText;
}

const text = 'photo=[{\\"markers_restarted\\":true,...'; 
